...
function minimumMoves(grid, startX, startY, goalX, goalY) {
    ...
    while (queue.length > 0) {
        ...
        for (let y = position.y - 1; y >= 0; y--) { //Left
            if (!markCellAsVisited(position.x, y, moves, currentCell)) break;
        }
        for (let x = position.x - 1; x >= 0; x--) { //Top
            if (!markCellAsVisited(x, position.y, moves, currentCell)) break;
        }
        for (let y = position.y + 1; y < grid.length; y++) { //Right
            if (!markCellAsVisited(position.x, y, moves, currentCell)) break;
        }
        for (let x = position.x + 1; x < grid.length; x++) { //Bottom
            if (!markCellAsVisited(x, position.y, moves, currentCell)) break;
        }
    }
    return 0;    
